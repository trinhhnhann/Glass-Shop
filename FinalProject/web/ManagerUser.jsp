<%-- 
    ...
    Created on : Jun 21, 2024, 6:19:40 PM
    Author     : nhana
--%>
...
<html>
    <head>
        ...
        <script type="text/javascript">
            function doDelete(userId) {
                if (confirm("Are you sure to delete User with UserID = " + userId + "?")) {
    ...
    <body>
        ...
        <form action="Controller" method="post">
            <div class="tag">
                <p>Home / User Manage</p>
            </div>
        </form>
        <div class="image">
            <form action="SearchUserByFullName" method="post">
                <img class="" src="IMG/INTRO/FindIcon.jpg" >           
                <input type="text" placeholder="Search by Full Name" name="query" /> 
            </form>
        </div>
        <div class="tabl">
            <table border="1px">
                ...
                <form action="Controller" method="POST">
                    <tr>
                        <td><input type="text" name="userId" value="<%= i.getUserId()%>"/></td>
                        <td><input type="text" name="fullName" value="<%= i.getFullName()%>"/></td>
                        <td><input type="text" name="roleId" value="<%= i.getRoleId()%>"/> </td>
                        <td><input type="text" name="email" value="<%= i.getGmail()%>"/></td>               
                        <td><input type="text" name="address" value="<%= i.getAddress()%>"/></td>
                    <input type="hidden" name="status" value="<%= i.getStatus()%>"/>
                    <input type="hidden" name="password" value="<%= i.getPassword()%>"/>
                    <td><button><a href="#" onclick="doDelete('<%=i.getUserId()%>')">DELETE</a></button></td>
                    <td>
